...
def render_decumulation_module():
    # Ladowanie globalnych parametrow
    _gs = get_gs()
    apply_gs_to_session(_gs)

    st.markdown("# 💸 Dekumulacja i dochód emerytalny")
    ...
    with st.sidebar:
        ...
        initial_cap = st.number_input("Kapitał startowy (PLN)", value=1000000, step=100000, key="dec_init_cap")
        mu_port = st.slider("Oczekiwana (nominalna) stopa zwrotu", 0.0, 0.15, 0.07, key="dec_mu")
        vol_port = st.slider("Zmienność portfela (%)", 0.0, 0.30, 0.12, key="dec_vol")
    ...
    with tab1:
        ...
        with col1:
            withdrawal_rate = st.slider("Początkowa stopa wypłaty (SWR)", 0.02, 0.08, 0.04, 0.005, format="%.3f", key="dec_swr")
        with col2:
            retirement_years = st.slider("Długość trwania emerytury (Lata)", 10, 50, 30, 5, key="dec_years")
    ...
    with tab2:
        ...
        with v_col1:
            ret_age = st.slider("Wiek przejścia na emeryturę", 50, 75, 65, key="dec_ret_age")
        with v_col2:
            max_age_val = st.slider("Oczekiwana długość życia (Max Age)", 85, 110, 100, key="dec_max_age")
    ...
    with tab3:
        ...
        const_w = initial_cap * 0.08 
        
        for y in range(years_seq):
            pb = path_bad[-1] - const_w
            if pb > 0: pb = pb * (1 + returns_seq[y])
            else: pb = 0
            path_bad.append(pb)
            
            pg = path_good[-1] - const_w
            if pg > 0: pg = pg * (1 + returns_good[y])
            else: pg = 0
            path_good.append(pg)
